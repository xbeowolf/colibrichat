...
#ifdef __cplusplus
#define ECRYPTNC ecrypt::edon80::
using namespace ecrypt::edon80;
#else
#define ECRYPTNC
#endif

const char* ECRYPTNC ECRYPT_NAME = "Edon80";           /* [edit] */ 
const char* ECRYPTNC ECRYPT_PROFILE = "___H3";
...
void ECRYPTNC ECRYPT_ivsetup(ECRYPT_ctx* ctx, const u8* iv)
{
...
void ECRYPTNC ECRYPT_encrypt_bytes(ECRYPT_ctx* ctx, const u8* plaintext, u8* ciphertext, u32 msglen)
{
	u32 i, j;       /* Variables i and j are internal counters. */
...
void ECRYPTNC ECRYPT_decrypt_bytes(ECRYPT_ctx* ctx, const u8* ciphertext, u8* plaintext, u32 msglen)
{
	u32 i, j;       /* Variables i and j are internal counters. */
	u8 X;           /* We will store produced byte from the keystream in X. */
