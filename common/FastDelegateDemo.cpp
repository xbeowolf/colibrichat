...
int main(void)
{
	...
	// Here a list for future batch call
	FastDelegateList2<int, char *> ev;
	...
	ev += funclist[0];
	ev -= MakeDelegate(&a, &CBaseClass::SimpleMemberFunction);
	...
	for (int i=0; i<12; i++) {
		...
		// Insert into delegates list
		if (i <= 9) ev += funclist[i];
	}
	// Make batch call to all delegates at once
	printf("\n-- Call to list: --\n\n");
	ev(-1, msg);
	return 0;
}
